...
const config = {
    ...
    trailLength: 0.8,
    stopDecay: 0.85,
    color1: '#769E5B', // Darker Pistachio Green
    color2: '#97B080', // Darker Light Pistachio
    color3: '#D9D7A8', // Darker Cream
    color4: '#3E4E22', // Deeper Olive Green
    colorIntensity: 0.9,
    softness: 1.0,
    lerpFactor: 0.1,
};
...
function FluidSimulation() {
    ...
    // Create a separate scene for the simulation to avoid it showing up in the main render
    ...
    useFrame(({ gl, clock, camera }) => {
        ...
        if (fluidMaterial.current && fluidMeshRef.current) {
            ...
            // Render the simulation scene to the off-screen buffer
        ...
        if (displayMaterial.current && displayMeshRef.current) {
            // Update the display material with the latest simulation result
            displayMaterial.current.uniforms.iFluid.value = currentFluidTarget.texture;
            // No need to manually render displayMeshRef here, React-Three-Fiber will render the default scene
        }
